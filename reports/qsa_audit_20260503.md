...
|---|---|---|
| `R001-invalid-timestamps` | 🔴 1 | Invalid / impossible timestamps in news tables |
| `R002-future-dates` | 🔴 1 | Future-dated rows where the column should be historical |
...
### `R001-invalid-timestamps` — masd.finnhub_news_articles

**2 row(s) with invalid/impossible published_at_utc (NULL or before 2000-01-01)**

NULL published_at_utc: 0 rows | published_at_utc before 2000-01-01: 2 rows | total rows: 87,280
...
_Recommendation:_ Add ingest validation rejecting/quarantining rows with published_at_utc before 2000-01-01 or NULL. Forward-only fix is fine for first pass; backfill quarantine in a later phase.
...
### `R002-future-dates` — masd.fmp_stocks_insider_trades

**632 row(s) with future-dated bar_date (max=2031-03-03)**

632 rows in masd.fmp_stocks_insider_trades have bar_date more than 1 day(s) in the future (across 193 distinct date(s)).
...
_Recommendation:_ Add ingest validation rejecting rows with future-dated transaction/filing/bar columns. Existing bad rows should be quarantined in a later phase.
